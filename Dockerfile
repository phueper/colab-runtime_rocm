...
deb [arch=amd64 signed-by=/etc/apt/keyrings/rocm.gpg] https://repo.radeon.com/amdgpu/${ROCMVERSION}/ubuntu jammy main
EOF
...
RUN sudo mkdir --parents --mode=0755 /etc/apt/keyrings \
    ...
    && sudo apt update \
    && apt-get install -y rocm amdgpu-lib \
    && apt-get clean \
